...
\begin{figure}[ht]
   ...
   \subfloat[Retro-alimentação positiva devido ao efeito corona.]{\includegraphics[width=5.9cm]{img/ilustracoes/eleconvectiva-c} \label{fig:elec-c}}
...
As colisões ocorrem sobre a influência de um campo elétrico $\mathbf{E}$ já existente na nuvem, provocando a polarização do \textit{graupel} e dos cristais de gelo conforme na figura \ref{fig:ind-a}. Ao colidirem, cargas são transferidas, tornando o \textit{graupel} carregado com carga negativa e os cristais de gelo com carga positiva ou falta de elétrons, conforme é mostrado na ilustração referente a figura \ref{fig:ind-b}.

\begin{figure}[ht]
   \centering
   \subfloat[Indução elétrica causada por um campo elétrico $\mathbf{E}$.]{\includegraphics[height=10cm]{img/ilustracoes/indutivo-a} \label{fig:ind-a}}  
   \subfloat[Colisão entre o \textit{graupel} e o cristal de gelo  sobre a influencia de $\mathbf{E}$.]{\includegraphics[height=10.9cm]{img/ilustracoes/indutivo-b} \label{fig:ind-b}}
   \caption{Ilustração da eletrização de hidrometeoros durante uma colisão sob a influência de indução elétrica.}
   \label{fig:ind}
\end{figure}
...
\simbolo{name={{\AA}},description={Ângstron}}
