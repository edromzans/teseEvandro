...
%Para estudar os processos de crescimento de hidrometeoros com base na morfologia da precipitação tridimensional das tempestades elétricas observadas pelo PR,

Para converter os níveis de altitude do PR em níveis de temperatura, utilizou-se os dados de reanálises (R2) do NCEP--DOE entre 1998--2011 em 17 níveis de pressão, referentes a altura geopotencial e a temperatura  correspondentes aos horários mais próximos e anteriores ao horário em que cada tempestade elétrica foi observada pelo TRMM. A partir dos 17 níveis verticais das R2, os 80 níveis de temperaturas associados aos 80 níveis de altitude das observações do PR foram calculados através de um método de mínimos quadrados.

Após ordenar as medidas de $Z_c$ do PR referente a cada tempestade elétrica em função de um eixo de temperatura, foi elaborado o Diagrama de Contorno de Frequência por Temperatura (CFTD\footnote{Sigla criada a partir da expressão em inglês, \textit{Contour Frequency Temperature Diagram}, nome semelhante ao CFAD proposto em \citeonline{yuter1995}}).

\sigla{name={CFTD},description={Diagrama de Contorno de Frequência por Temperatura, do inglês, \textit{Contour Frequency Temperature Diagram}}} 
...
Também foi calculada a função $f_{cdf}(x,y)$ de $Z_c$ por temperatura, que originaram os Diagramas de Contorno de Frequência Cumulativa por Temperatura (CCFTD).  \sigla{name={CCFTD},description={Diagrama de Contorno de Frequência Cumulativa por Temperatura, do inglês, \textit{Contour Cumulative Frequency by Temperature Diagram}}}   
